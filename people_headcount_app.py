...
dept_weight = st.sidebar.slider("Department/skill weight", min_value=0.0, max_value=5.0, value=0.5, step=0.1)

# Advanced: JSON overrides for weights (optional)
with st.sidebar.expander("Advanced: JSON weight overrides", expanded=False):
    json_override = st.text_area(
        "Paste JSON to override weights (e.g. {\"comp_weight\":1.2, \"tenure_weight\":0.4})",
        value="",
        height=120,
    )
    try:
        if json_override.strip():
            parsed = json.loads(json_override)
            comp_weight = float(parsed.get("comp_weight", comp_weight))
            tenure_weight = float(parsed.get("tenure_weight", tenure_weight))
            level_weight = float(parsed.get("level_weight", level_weight))
            reports_weight = float(parsed.get("reports_weight", reports_weight))
            dept_weight = float(parsed.get("dept_weight", dept_weight))
            st.sidebar.success("Applied JSON overrides")
    except Exception:
        st.sidebar.error("Invalid JSON overrides; ignoring.")
...
def compute_department_skill_score(role_series: pd.Series, department_series: pd.Series) -> pd.Series:
    dept_base = {
        "Engineering": 1.2,
        "Product": 1.0,
        "Sales": 0.9,
    ...
    def score_for_row(role: str, dept: str) -> float:
        base = dept_base.get(dept, 1.0)
        r = (role or "").lower()
        if "ml" in r or "ai" in r or "research" in r:
            base *= 1.2
        if "security" in r:
            base *= 1.15
        if "devops" in r or "infrastructure" in r or "platform" in r:
            base *= 1.05
        if "data" in r or "scientist" in r:
            base *= 1.1
        return base
    return pd.Series([score_for_row(r, d) for r, d in zip(role_series.fillna(""), department_series.fillna(""))])
...
roster_df["dept_skill_score"] = compute_department_skill_score(roster_df.get("role", pd.Series([""] * len(roster_df))), roster_df.get("department", pd.Series([""] * len(roster_df))))
...
dept_norm = roster_df["dept_skill_score"] / max(1.0, roster_df["dept_skill_score"].max())
